...
- Context fields: `requestedDate`, `resolvedDate`, `dataWindow.currentPeriod`, `thresholdBasis`, marketplace/site/granularity
- Item fields: `identity`, `status`, `marketProfile`, `emptyReason`, `errorCode`, `errorMessage`
- `marketProfile` dimensions: `marketContext`, `demandScale`, `top3Concentration`, `adActivity`, `top20OrganicEntryDifficulty`, `supplySaturation`, `brandStructure`, `organicProductBenchmark`, `calculationCoverage`
- Interpret returned levels/scores only with the returned threshold/coverage context. Profile objects currently use snake_case internal keys. Check each `calculationCoverage.dimension_coverage[]` entry: top-level `supported=true` does not mean every dimension is supported. This is weekly snapshot evidence, not trend, root-cause, recommendation, or seller-private ABA-SQP data.
