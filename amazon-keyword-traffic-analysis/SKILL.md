...
2. Read the local `references/cli-result-contract.md`, `references/reference.md`, and the relevant `zoodata.py --help` before selecting a tool. The shared contract owns CLI result handling; `reference.md` is the sole source for production endpoint availability, parameters, response fields, dates, batching, credits, and API capability boundaries.
