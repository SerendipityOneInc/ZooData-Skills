...
- Treat `market-profile` explicitly as a **metric-layer endpoint** derived from the same weekly snapshot source as the data-layer `detail` endpoint. Preserve `context`, per-item status, `marketProfile`, `thresholdBasis`, and `calculationCoverage`; never present its levels/scores without the returned threshold and coverage context. Check every `calculationCoverage.dimension_coverage[]` entry—top-level `supported=true` does not mean every profile dimension is supported.
...
7. Do not descend merely because a metric item is `empty`, a dimension is `unknown`, or `calculationCoverage` is incomplete. First determine whether the data layer contains additional—not merely identical upstream—evidence.
...
8. `keywords/market-profile` returns `data.context + data.items[]`; use its server-calculated `marketProfile` dimensions as snapshot evidence only. Profile objects currently contain snake_case internal keys such as `input_field_names`, `unsupported_reason`, and `dimension_coverage`. A dimension with `level=unknown`, `score=null`, or non-null `unsupported_reason` is unavailable even when the overall coverage object says `supported=true`. It does not provide history, recommendations, root cause, or seller-private conversion data. The endpoint is currently pre-release on localhost; check the live surface before calling it elsewhere.
