...
| Metric layer `keywords/market-profile` when exposed on the target surface | Server-calculated weekly demand scale, Top3 concentration, ad activity, organic-entry difficulty, supply saturation, brand structure, organic benchmark, and calculation coverage; only dimensions supported by `dimension_coverage[]` | Mark unsupported metric dimensions unavailable. Use `detail` only if it exposes different raw evidence required for a named inference; do not assume it can repair missing metric inputs |
