...
| ASIN-wide multi-week trend or trajectory | `product-traffic-trend-profile` when no window is specified or the requested window matches the profile-supported window; `product-traffic-trend` for any explicit non-matching window, whether shorter or longer, or when raw weekly points are requested | Do not stretch the profile conclusion across a different window; keep every raw request within the range contract in `reference.md` |
...
- ASIN-wide profile conclusion for its supported window, or raw weekly trend for a different explicitly requested supported window.
...
- Stage 1A uses the metric profile when no trend window is specified or the requested window matches the profile-supported window. Use raw `product-traffic-trend` for an explicit window that does not match, including a shorter window, and for required weekly points or detail omitted by the metric. Do not ask the user to restate a valid shorter-window request, stretch the profile across a different window, or relabel an Agent reading of raw points as the server profile. Preserve item/row status, coverage, resolved period/window scope, returned modules, fields, and evidence; do not reconstruct the profile from traffic rows.
