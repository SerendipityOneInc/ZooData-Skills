...
Use `totalMonthlyRevenue` from `markets/search` for full-category revenue and `top100MonthlyRevenue` for its selected sample. Do not calculate revenue from price × sales; the field definitions are in `reference.md § 2`.
