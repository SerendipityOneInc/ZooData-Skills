...
5. Treat an execution tool's display truncation marker, omitted middle, or incomplete displayed row set as a conversation-rendering limit, not API evidence. Do not analyze or rank from that transcript. Re-project from the complete isolated raw file within the same run. A returned count such as `meta.total` does not prove that every row and required field was inspected. If the complete raw result is no longer available, suppress the completeness-dependent conclusion rather than inferring the omitted content.
