...
The shared CLI uses `market` for both `markets/search` discovery and exact `categoryId` snapshots, plus `market-structure-profile` and `market-history` for the remaining market endpoints. The experimental `markets/overview` path and `market-overview` CLI command were intermediate artifacts of this work, were never published, and have been removed from the final interface. Composite market context resolves `categoryId`, queries `markets/search` with `pageSize=1`, and reads the matching row; skill references, examples, and routing tests use the `total*` and selected `sample*` field families. Retired market CLI flags (`--category`, `--keyword`, `--topn`, `--top100-*`) were removed. The CLI exposes current `--sample-*` filters and `sampleMonthlySales` / `sampleMonthlyRevenue` sorting. The current MCP `markets/search` request groups row selectors under `category` (`ids`, `path`, or `name`), per-row descendant-product inclusion under `category.includeDescendantCategoryProducts`, and metric conditions under `filters`. The CLI exposes `--category-ids` for up to 100 markets per call and removes `market --scope`; structure-profile and history retain their separate scope option. Parent-category discovery now obtains every direct child ID before batch market comparison. The server still accepts legacy flat requests separately, but the bundled CLI sends only the nested form.
