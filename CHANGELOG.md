...
`amazon-market-analysis` connects market and product discovery, named-market entry assessment, and category-change tracking in one independently routed skill. Its concise `SKILL.md` declares module ownership; scenario, evidence, metric, seller-input, output, and production API contracts live in separate references. It uses `markets/search` for exact category snapshots and the new structure-profile and history endpoints. It is the unified market skill. The three previous market-entry, trend-scanner, and opportunity-discoverer directories remain as retired source, with `RETIRED.md` replacing their discoverable `SKILL.md` entrypoints so installers and publishers expose only the 10 active skills. Current market responses use the selected `sample*` field family; the skill and supporting references distinguish it from full-category `total*` measures.

Completed market stages now close with evidence-backed, self-contained follow-up choices and a final new-question/exit choice. The handoff Gate rejects unsupported leaps, leaves the next stage to the user's selection, and reclassifies each actual reply. The report renderer puts this single numbered list after API usage instead of relying on a prose-only next step.

Live evidence commands now default to a private, per-run temporary raw-result file followed by local claim-specific projection. Results live under a current-user root, ISO-date bucket, and random run directory. This prevents execution-tool display truncation from silently becoming incomplete evidence. The shared caller contract requires immediate cleanup on success, failure, projection errors, and catchable interruption. As a nonblocking backstop, CLI startup schedules a detached worker at most daily to remove whole date buckets older than 30 days, covering abandoned data after untrappable termination or host failure without touching active, non-date, symlink, or unrelated temporary entries. Market comparison rules validate every requested category and ranking field against the complete raw result before ranking, without repeating a paid API call.
...
Market-history reports now interpret relative recent-month requests as completed month-end periods and omit the current incomplete month entirely. Trend tables contain only returned points whose dates and displayed fields were validated against the complete isolated result. Confirmed source gaps are named precisely in Data Notes; when expected coverage is complete, missingness commentary and defensive explanations of valid zero values are omitted. Vague placeholders such as "details unconfirmed" or "incomplete response" are rejected. A local projection or display omission is recovered from the existing raw result without another paid query and cannot be presented as missing API data.

Added a canonical Shared Analysis Contract and synced it into the keyword and market analysis skills as the first adopters. It owns the repository rule hierarchy, universal evidence and authority Gates, continuation discipline, user-facing implementation boundary, and whole-draft Final Response Gate. Its structured evidence model keeps present values (including zero, false, and empty values), explicit nulls, absent fields, unreturned subjects or periods, and locally unread values distinct. Documented, contract-conforming data is the basis for analysis and judgment without speculative quality caveats or independent upstream audits; the Agent questions data only when the acquired evidence contains a concrete contradiction or contract violation. Keyword and market modules now keep only their domain routing, evidence, semantics, scenario, and rendering rules; they may narrow the shared contract but cannot duplicate, weaken, or override it.

### Changed — Markets API v2 upgrade

The shared CLI uses `market` for both `markets/search` discovery and exact `categoryId` snapshots, plus `market-structure-profile` and `market-history` for the remaining market endpoints. The experimental `markets/overview` path and `market-overview` CLI command were intermediate artifacts of this work, were never published, and are absent from the active tool list. Composite market context resolves `categoryId`, queries `markets/search` with `pageSize=1`, and reads the matching row; skill references, examples, and routing tests use the `total*` and selected `sample*` field families. The current MCP `markets/search` request groups row selectors under `category` (`ids`, `path`, or `name`), per-row descendant-product inclusion under `category.includeDescendantCategoryProducts`, and metric conditions under `filters`. The CLI exposes `--category-ids` for up to 100 markets per call, the complete current filter and sort surface, and explicit `--top-n` / `--new-product-period` selectors. Structure-profile and history now use top-level `includeDescendantCategoryProducts` instead of `categoryScope`; history uses `dateFrom` / `dateTo` instead of `startDate` / `endDate`. The CLI follows those new contracts directly through its boolean descendant flag and `--date-from` / `--date-to`, without legacy aliases. Parent-category discovery still obtains every direct child ID before batch market comparison.
...
All active local API summaries now use the same Market API request and response names as the canonical reference. The market router contains only routing, loading, ownership, and cross-module boundaries; API syntax stays in `reference.md`, field and time meaning stays in `market-metric-semantics.md`, evidence mechanics stay in `evidence-protocols.md`, rendering stays in `output-rules.md`, and scenarios retain only their entry evidence and conclusion scope. One-time supplied-snapshot comparisons route to historical trend, while persistent watch entry requires explicit monitoring intent, cadence, thresholds, and baseline authorization.

The general analysis operations guide now routes product-level history to `history` and category-market history to `market-history`; compatible saved snapshots or peer baselines are reserved for dimensions without corresponding history data. Listing-audit and review-intelligence discovery descriptions now say they use up to 11 relevant ZooData endpoints rather than implying that ZooData has only 11 endpoints.

The 2026-09-20 market contract replaces fixed six-month new-product fields and `newProductMetrics[]` with a request-selected `newProductPeriod` (`1`, `3`, `6`, or `12`) plus full-category `totalNewProduct*` and selected Top 100 `sampleNewProduct*` values. `topN` selects the window used by dynamic Top N filters and sorting, while `topNMetrics[]` preserves each returned `n`. Structure buckets now include Amazon self-operated measures, selected-period new-product measures, and example ASINs. Market history echoes the chosen period and returns `resolvedDateFrom` / `resolvedDateTo`. Scenario rules use every concentration and new-product value with its returned denominator and window, without inherited fixed cutoffs. The general-analysis API usage template reads accumulated CLI `meta.apiCalls`, `meta.creditsConsumed`, and `meta.creditsRemaining` instead of treating a composite CLI execution as one endpoint call.

### Chore — Patch-version bump across active skills for ClawHub republish

All active skills received a patch bump so the fixes in this cycle (composite category-resolution metadata, ABA out-of-window date guidance, the standardized `resolved_category_path` meta key, CLI allowlist enforcement, credential-source hardening, and the SKILL.md description trims) propagate to installed users on `openclaw skills update`. The shared `zoodata.py` change touches every active `amazon-*` skill's synced copy, so the bump is repo-wide rather than per-skill.

### Fixed — Composite category resolution is no longer misread as missing data

Keyword-driven composites (`report`, `market-entry`, `competitor-analysis`, `pricing-analysis`, `daily-radar`, `listing-audit`, `opportunity-scan`, `review-deepdive`) resolve the working category through a fallback chain. For a multi-word product phrase (e.g. `"yoga mat"`) the first step — `categories`/search, which matches category *names* — returns empty, and the category is instead resolved from the top product's `categoryPath`. The bundle then carried an empty top-level `categories` section that read like missing data, even though `meta.category_source` recorded the fallback. The composite `meta` now also carries `resolved_category_path` (the path actually used, or `null` when genuinely unresolved), so an empty `categories` section paired with a non-null path is unambiguously successful fallback resolution. `cli-contract.md` § *Command identity and composite reuse* documents the signal; hermetic tests cover the fallback and unresolved cases. Live-verified: `report --keyword "yoga mat"` reports `category_source: inferred_from_search` and `resolved_category_path: ["Sports & Outdoors","Exercise & Fitness","Yoga","Mats"]`. The 7 composite bodies that previously wrote the resolved path under an undocumented legacy key (`meta.resolved_category`) now use the single documented name `meta.resolved_category_path`, so every composite — including ASIN-first paths like `pricing-analysis` — carries the field consistently (live-verified on pricing: `category_source: asin_realtime`, path populated). A drift-guard test forbids the legacy key from returning.
