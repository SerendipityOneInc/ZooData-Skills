...
- `newProductPeriod` selects a `1`, `3`, `6`, or `12` calendar-month business launch window. A product is new when its business launch date is later than the snapshot date minus that many calendar months and no later than the snapshot date. A product without a business launch date is not counted as new but remains in the applicable product-count denominator.
- `totalNewProductCount` and `totalNewProductRate` describe that selected window across the full category scope. `sampleNewProductCount`, `sampleNewProductRate`, `sampleNewProductMonthlySales`, and `sampleNewProductMonthlyRevenue` describe it within the selected Top 100; the sample rate uses `sampleSkuCount` as its denominator. Always carry the returned `newProductPeriod` with these fields. A broader claim about all entrants, survival, or seller success is a different concept and needs its own evidence.
...
- Search `topN` chooses the window used by dynamic `top*` filters and sorting; it does not change the `n` identity of each returned `topNMetrics[]` row. Fixed `sampleTop10*` fields remain specifically Top 10 regardless of `topN`.
- `markets/structure-profile.data.buckets[]` partitions one selected Top 100 dimension for the returned date. Its `skuRate`, `salesRate`, and `revenueRate` have different denominators. Bucket `amazonSelfOperated*` fields describe the Amazon self-operated subset inside that bucket, and bucket `newProduct*` fields use the response's selected `newProductPeriod`. An example ASIN is an illustration of its bucket, not proof that the entire category has that property.
